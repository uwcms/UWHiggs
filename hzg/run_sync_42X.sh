...
cmsRun  $theCfg inputFiles="$mcSignalEle" outputFile="MCSignalEle_42X_ntuples.root" makeHZG=1 makeTNP=1 passThru=1 reportEvery=100 >& MCSignalEle_ntuples.log &

cmsRun  $theCfg inputFiles="$mcSignalMuo" outputFile="MCSignalMuo_42X_ntuples.root" makeHZG=1 makeTNP=1 passThru=1 reportEvery=100 >& MCBkgEle_ntuples.log &

cmsRun  $theCfg inputFiles="$dataElectron" outputFile="DataElectron_42X_ntuples.root" makeHZG=1 makeTNP=1 passThru=1 reportEvery=100 >& DataElectron_ntuples.log &

cmsRun  $theCfg inputFiles="$dataMuon" outputFile="DataMuon_42X_ntuples.root" makeHZG=1 makeTNP=1 passThru=1 reportEvery=100 >& DataMuon_ntuples.log &
