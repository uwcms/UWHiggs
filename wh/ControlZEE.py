...
def make_weight(fcn):
    def f_(*args):
        ...
        try:
            ...
            raise ZeroDivisionError(" catched trying to return weight for %s in %s" % (args.__repr__, fcn.__name__))
    return f_
...
def assign_charge_weight(dir_, row):
...
class ControlZEE(MegaBase):
    ...
    def preselection(self, row):
        ...
        if region == 'wjets' or region == 'qcd': return False 
        return True
    ...
    def process(self):
        ...
        for row in self.tree:
            ...
            matchingd = filter(lambda x: x.startswith(pair_sign+'/'+id1+id2), self.dirs)
