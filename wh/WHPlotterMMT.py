...
if __name__ == "__main__":
    ...
    if not options.dry_run:
        ...
        for label, proj_range in categories.iteritems():
            ...
            #DR
            plotter.plot_final_f3("m1_t_DR#LT", 5, xaxis='#DeltaR_{#mu_{1}#tau}', maxy=None, project=proj_range, project_axis='X')
            plotter.add_cms_blurb(sqrts)
            plotter.save('final-f3-m1_t_DR-%s' % label)

            plotter.plot_final_f3("m2_t_DR#LT", 5, xaxis='#DeltaR_{#mu_{2}#tau}'  , maxy=None, project=proj_range, project_axis='X')
            plotter.add_cms_blurb(sqrts)
