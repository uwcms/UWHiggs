...
## 
    def begin(self):
        ...
        self.book('os/gg/ept30/', "h_vismass",  "h_vismass",  32, 0, 320)
    ...
    def fill_histos(self, row, f='os/gg/ept0/0',  isTauTight=False, frw=[1.,1.,1.]):
        ...
        if f=='os/gg/ept30' :
            if bool(isTauTight)==True:
                histos[f+'/h_collmass_pfmet'].Fill(collmass(row, row.type1_pfMetEt, row.type1_pfMetPhi), weight[0])
                histos[f+'/h_vismass'].Fill(row.e_t_Mass, weight[0])
        else:
