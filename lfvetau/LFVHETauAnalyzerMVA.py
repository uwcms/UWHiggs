...
## 
    def begin(self):
        ...
        jetN = [0, 1, 2, 3]
        folder=[]
        pudir = ['','p1s/', 'm1s/','trp1s/', 'trm1s/', 'eidp1s/','eidm1s/',  'eisop1s/','eisom1s/', 'tLoose/','tLooseUp/','tLooseDown/', 'tLooseUnweight/' ]
    ...
    def fill_histos(self, row, f='os/gg/ept0/0',  isTauTight=False, frw=[1.,1.,1.]):
        ...
        if bool(isTauTight) == False:
            #print 'isTauTigh? ', isTauTight, '  in ', f
            ...
            #print frweight_bv, frw[0], row.tPt, row.tEta
    ...
    def process(self):
        ...
        for row in self.tree:
            ...
            for j in ptthreshold:
                ...
                #if (row.run, row.lumi, row.evt, row.ePt, row.tPt)==myevent: continue
                ...
                #myevent=(row.run, row.lumi, row.evt, row.ePt, row.tPt)
                ...
                #print row.run, row.lumi, row.evt, row.ePt, row.tPt, row.tMtToPFMET,  row.eEta, row.tEta, row.ePhi, row.tPhi                 
                #print 'histo filled'
                #selections
                if jn == 0 :
